...
void simple_net(dnnl::engine::kind engine_kind, int32_t times)
{
    ...
    std::vector<std::unordered_map<int32_t, memory>> net_args;
    ...
    /// for weights.
    memory user_src_memory = memory({{conv1_src_tz}, dt::f32, tag::nchw}, eng);
    write_to_dnnl_memory(user_src.data(), user_src_memory);
    memory user_weights_memory = memory({{conv1_weights_tz}, dt::f32, tag::oihw}, eng);
    write_to_dnnl_memory(conv1_weights.data(), user_weights_memory);
    memory conv1_user_bias_memory = memory({{conv1_bias_tz}, dt::f32, tag::x}, eng);
    ...
    if (conv1_prim_desc.weights_desc() != user_weights_memory.get_desc()) {
        ...
        reorder(user_weights_memory, conv1_weights_memory)
                .execute(stm, user_weights_memory, conv1_weights_memory);
    }
    ...
    if (conv2_prim_desc.weights_desc()
        ...
        reorder(conv2_user_weights_memory, conv2_weights_memory)
                .execute(stm, conv2_user_weights_memory, conv2_weights_memory);
    }
    ...
    if (conv3_prim_desc.weights_desc()
        ...
        reorder(conv3_user_weights_memory, conv3_weights_memory)
                .execute(stm, conv3_user_weights_memory, conv3_weights_memory);
    }
    ...
    if (conv4_prim_desc.weights_desc()
        ...
        reorder(conv4_user_weights_memory, conv4_weights_memory)
                .execute(stm, conv4_user_weights_memory, conv4_weights_memory);
    }
    ...
    if (conv5_prim_desc.weights_desc()
        ...
        reorder(conv5_user_weights_memory, conv5_weights_memory)
                .execute(stm, conv5_user_weights_memory, conv5_weights_memory);
    }
    ...
    if (fc6_prim_desc.weights_desc() != fc6_user_weights_memory.get_desc()) {
        ...
        reorder(fc6_user_weights_memory, fc6_weights_memory)
                .execute(stm, fc6_user_weights_memory, fc6_weights_memory);
    }
    ...
    if (fc7_prim_desc.weights_desc() != fc7_user_weights_memory.get_desc()) {
        ...
        reorder(fc7_user_weights_memory, fc7_weights_memory)
                .execute(stm, fc7_user_weights_memory, fc7_weights_memory);
    }
    ...
    if (fc8_prim_desc.weights_desc() != fc8_user_weights_memory.get_desc()) {
        ...
        reorder(fc8_user_weights_memory, fc8_weights_memory)
                .execute(stm, fc8_user_weights_memory, fc8_weights_memory);
    }
    ...
    for (int32_t j = 0; j < times; ++j) {
        ...
        for (size_t i = 0; i < net.size(); ++i)
            net.at(i).execute(stm, net_args.at(i));
    }

    stm.wait();
}

void cnn_inference_f32(dnnl::engine::kind engine_kind)
{
...
TEST_CASE("Test CNN" "[oneDNN]")
{
    cnn_inference_f32(dnnl::engine::kind::cpu);
}
